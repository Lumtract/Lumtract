...
/* ── 九 · 导航与流转（N-系列，推导完整性）─────────────────
 *
 * 【工具纪律 · ADR-0002 §2.3 / ADR-0003 §2.1 #1】
 * 本组只校验「推导完整性」，不校验 [ENG] 阈值：
 *   可断言：派生 ID 挂得住、平台行为已登记 §2.6、来源类型非空、
 *           硬度-来源一致（硬度含钻石 → 来源须含 [派生自]/[新原语]）
...
// 4) 硬度-来源一致性（ADR-0003 §2.1 #1）：
//    硬度含「钻石」→ 来源类型必须含 [派生自] 或 [新原语]。
//    硬度栏说钻石、来源栏只有 [ENG] = 半句无锚的隐蔽冒充，判阻断。
//    纯陶土（[ENG]）不适用本规则——机器不得把合规条款升级成生理边界。
for (const [id, r] of rowById) {
  if (!/钻石/.test(r[3])) continue;
  const ok = /派生自|新原语/.test(r[2]) ? 1 : 0;
...
console.log('\n' + C.b + 'Lumtract · 约束校验' + C.x + C.d + '  v10.3.0' + C.x + '\n');
